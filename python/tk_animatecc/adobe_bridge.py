...
class AdobeBridge(Communicator):
    ...
    SHOTGUN_ADOBE_HEARTBEAT_TIMEOUT = os.environ.get(
        ...
        os.environ.get(
            "SGTK_ANIMATE_HEARTBEAT_TIMEOUT",
            0.5,
        ),
    )
    ...
    def get_active_document(self):
        return self.rpc_eval("fl.getDocumentDOM() ? JSON.stringify(wrap_item(fl.getDocumentDOM(), fl.getDocumentDOM().name)) : undefined")
    ...
    def get_active_document_path(self):
        ...
        with self.response_logging_silenced():
            try:
                path = self.rpc_eval("fl.getDocumentDOM() ? fl.getDocumentDOM().path : undefined")
            except RuntimeError:
                path = None
    ...
    def save(self):
        self.rpc_eval("fl.saveDocument(fl.getDocumentDOM())")
    ...
    def save_as(self, file_path):
        ...
        """
        self.rpc_eval("fl.saveDocument(fl.getDocumentDOM(), '%s')" % self._flashify_path(file_path))

    def open(self, file_path):
        self.rpc_eval("fl.openDocument('%s')" % self._flashify_path(file_path))

    def close(self):
        self.rpc_eval("fl.closeDocument(fl.getDocumentDOM())")
